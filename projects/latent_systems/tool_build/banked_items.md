...
**Updated:** 2026-05-13 Day 1 of Phase 3 sprint (Phase 2.5 conftest fix LANDED commit 8be45e3 — tmp_path-isolated DB fixture per Pattern #8 reinforcement #3 + Banking principle #7; 15 test files migrated via `def test_main(): assert main() == 0` wrapper pattern preserving standalone invocation; full suite 94 passed + 1 documented skip (test_router_tail::test_main — _ingest_roundtrip needs real-repo file structure, covered by standalone path); production state.db row counts byte-identical pre/post pytest run. Test runner quirk banked 2026-05-09 RESOLVED — script-loop trap no longer applies under pytest mode; SESSION_STARTER state-check switched to `python -m pytest tests/ -q`. Workstream 1B rubric calibration scratch authored commit 2729160 at `tool_build/RUBRIC_CALIBRATION_SCRATCH_2026_05_13.md` — Documentary integrity sharpening draft with apparatus accuracy + period authenticity dimensions; pending Joseph review + integration into docs/AUDIT_RUBRICS_v1_0.md before Day 2 morning F5 work begins.) Prior session 2026-05-09: Phase 2 acceptance bridge COMPLETE (rubric a0bc668, Wave B real-fire, e2e debrief); Daily-usability sprint a32572f 2026-05-08; Phase 3 v0.4 amendments f7e0604/1065d75/ae48115/2411811. **Cleanup-leak class banking 2026-05-09** — Pattern #8 reinforcement #3 (test_audit_consult audit_sessions + test_vision_anthropic api_calls leaks); Banking principle #7 added; api_calls cleanup commit a52ec90 deleted 21 test-residue rows.
