...
- pass: Subject positioned for 9:16 crop with vertical headroom; key relationships (subject + apparatus, person + object) read at thumbnail scale; high contrast between subject and background.
- partial: Reads at-a-glance in 16:9 but 9:16 crop loses one element — apparatus or contextual frame extends off vertical crop. Recoverable with reframe-prompt iteration.
- fail: Subject lost in busy composition or too small in frame; critical relationships only readable at full resolution; low contrast between subject and background.
...
this channel.
- pass: Aesthetic aligns clearly with one of the 5 visual-identity directions (latent_space, architectural_inhabitant, composite_subject, schematic_apparatus, surreal_subject); palette and register match canonical exemplars.
- partial: Aligns with a direction but with off-brand drift — slightly hot saturation, conventional composition, or register that tilts toward performance. Recoverable with prompt tightening.
- fail: Reads as generic AI output without channel-specific identity; could be from any AI-native channel; doesn't anchor to any of the 5 directions.
...
`shared/visual_identity_phase1_references/README.md`).
- pass: Visual register lands at channel level — could appear in any of EP1-EP10 without feeling out of place; visual-identity decisions hold across subject-matter shifts.
- partial: Mostly reusable but one element ties specifically to current episode subject (period-specific detail, episode-specific iconography). Recoverable by abstracting or accepting episode-only scope.
- fail: Visual register pins so tightly to current episode's subject that it can't be reused; episode-specific in a way that costs cross-series continuity.
...
- pass: Generation parameters and prompt vocabulary documented; re-running produces register-consistent results; tool-grammar lessons fold into channel-level reproducibility.
- partial: Reproducible but inefficient — requires many regenerations or substantial iteration; vocabulary works but not yet documented at channel level. Recoverable by extracting and banking the lesson.
- fail: One-shot result that can't be reproduced reliably; prompt undocumented or unrepeatable; register held by accident rather than craft.
...
- pass: Visual reads as observational frame for Daniel's narration; doesn't compete with VO for attention; silent-by-default friendly; held-black moments hand off to silence cleanly.
- partial: Mostly subordinates to audio but one element pulls focus — a kinetic detail or animated motion catches attention away from VO. Recoverable by stilling or simplifying the offending element.
- fail: Visual demands narration-equal attention; pre-empts editorial beats by being dramatic on its own terms; competes with VO rather than supporting it.
